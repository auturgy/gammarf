...
VERSION_STRING = "GammaRF v0.1, Copyright 2016 (gammarf |at| covert.codes)"
...
def main():
    ...
    for devnum, dev in loadedmods['devices'].devs.iteritems():
        ...
        if isinstance(cmdline, str):
            ...
            if loadedmods[module].run(devnum, args, system_params, loadedmods):
                loadedmods['devices'].occupy(devnum, module, cmdline)
    ...
    while True:
        ...
        if isinstance(cmdline, str):
            ...
            if loadedmods[module].run(devnum, args, system_params, loadedmods):
                loadedmods['devices'].occupy(devnum, module, cmdline, pseudo=True)
...
class Interpreter(cmd.Cmd):
    ...
    def do_nodes(self, line):
        """Show currently active GammaRF client nodes (locations only)"""
    ...
    def do_run(self, line):
        ...
        if parsed[0] == '' or parsed[0] == '':
            print("Type 'help run' for command usage")
            return
        ...
        if self.loadedmods['devices'].reserved(devnum):
        ...
        if self.loadedmods[module].ispseudo():
            ...
            if not self.loadedmods['devices'].isdev(devnum):
                print("Not a device: {}".format(devnum))
                return

        if not self.loadedmods['devices'].occupied(devnum) or (module == 'remotetask' and (cmdline and cmdline.split(' ', 1)[0] == 'request') ):

            if self.loadedmods[module].run(devnum, cmdline, self.system_params, self.loadedmods):
                self.loadedmods['devices'].occupy(devnum, module, cmdline, pseudo)
        else:
